...
struct SettingsView: View {
    // Stores user settings for difficulty, sound, and game lock
    @AppStorage("difficulty") private var difficulty = "Medium"
    @AppStorage("soundEnabled") private var soundEnabled = true
    @AppStorage("gameLockEnabled") private var gameLockEnabled = true
    @AppStorage("hintsEnabled") private var hintsEnabled = true  // New setting for hints
    
    // Accesses the gameManager shared across the app
    @EnvironmentObject var gameManager: GameManager
    
    var body: some View {
        NavigationView {
            // Form to organize settings options
            Form {
                // Picker to select game difficulty level
                ...
                // Toggle to enable or disable sound effects
                Toggle("Sound Effects", isOn: $soundEnabled)
                
                // Toggle to enable or disable sound effects
              
                // Toggle to enable or disable game locking feature
                Toggle("Lock Games", isOn: $gameLockEnabled)
                    .onChange(of: gameLockEnabled) { newValue in
                    
                        // Updates game lock setting in gameManager when toggled
                        gameManager.setGameLock(enabled: newValue)
                    }
                Toggle("Enable Hints", isOn: $hintsEnabled)  // New toggle for hints
                
            }
            // Sets the title of the settings page
            .navigationTitle("Settings")
        }
        .onAppear {
            // Syncs gameManager's game lock setting with the stored value
                    ...
                    VStack {
                       ...
                       // Text(game.name)
                        ...
                        if currentQuestion < questions.count {
                            ...
                            let gridItemSize = (geometry.size.width - 70) / 2 // 40 is total horizontal padding
    ...
    func startTimer() {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { _ in
            if self.timeRemaining > 0 {
                ...
                self.checkAnswer(nil) // Time's up, treat as wrong answer
            }
        }
    }
